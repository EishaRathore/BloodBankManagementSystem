    ...
    class UserDAL
    {
        ...
        public bool Insert(UserbLL u)
        {
            ...
            try
            {
                ...
                string SQL = "INSERT INTO userTbl(userName,email,upassword,ucpassword,contact,uaddress,addDate,Gender,imSource) VALUES(@userName,@email,@upassword,@ucpassword,@contact,@uaddress,@addDate,@Gender,@imSource)";
        ...
        public bool Valid(UserbLL u)
        {
            ...
            {
                //create the string variable to hold the sql query to get data
                string SQL = "SELECT * FROM [dbo].[UserTbl] WHERE (userName=@userName or email=@email) and (upassword=@upassword) ";
               ...
               cmd.ExecuteNonQuery();
                ...
                {
                    //failed to execute query
                    MessageBox.Show("Please Enter Correct User Name or Email or Passward");
                    isSuccess = false;
                }
            }
            catch (Exception Ex)
        ...
        #region
        public bool passvalidation(string str,Label lbl)
        {
            //  mail@mail.com                        => ^([\w]+)@([\w]+)\.([\w]+)$
            ...
            //Regexp(@"^([\w]+)@([\w]+)\.([\w]+)$", utextbox, invalid1, "Password");


            if (!hasLowerChar.IsMatch(str))
            {
                lbl.ForeColor = Color.Red;
                lbl.Text = "Password should contain at least one lower case letter.";
                return false;
            }
            else if (!hasUpperChar.IsMatch(str))
            {
                lbl.ForeColor = Color.Red;
                lbl.Text = "Password should contain at least one upper case letter.";
                return false;
            }
            else if (!hasMiniMaxChars.IsMatch(str))
            {
                lbl.ForeColor = Color.Red;
                lbl.Text = "Password should not be lesser than 8 or greater than 15 characters.";
                return false;
            }
            else if (!hasNumber.IsMatch(str))
            {
                lbl.ForeColor = Color.Red;
                lbl.Text = "Password should contain at least one numeric value.";
                return false;
            }
            

            else if (!hasSymbols.IsMatch(str))
            {
                lbl.ForeColor = Color.Red;
                lbl.Text = "Password should contain at least one special case character.";
                return false;
        ...
        #region convert binary source into image
